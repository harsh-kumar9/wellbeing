...
import { addConversationLog, updatePostSurveyResults, setSessionDetails } from '../reducers/sessionSlice';
import { langchainActions } from "../reducers/langchainSlice";
import { OpenAIChat, AzureChatOpenAI } from "langchain/llms/openai";
import { HuggingFaceInference } from "langchain/llms/hf";
import { PromptTemplate } from "langchain/prompts";
import { LLMChain } from "langchain/chains";
import { handleConversation, handlePostSurvey } from '../utils/sessionUtils';
...
// const model = new OpenAIChat({
//   temperature: 0, // determine how stochastic we want it to be, 0 for experimentation
//   azureOpenAIApiKey: "f50de5e0554840828df4fbd4518283be",
//   azureOpenAIApiVersion: "2024-02-15-preview",
//   azureOpenAIApiInstanceName: "azure",
//   azureOpenAIApiDeploymentName: "autogen",
// });
...
export const startTherapySession = (userId, studentPrompt, counselorPrompt, postQuestions) => {
  return async (dispatch) => {
      try {
          // Set session start details
          dispatch(setSessionDetails({
              startTime: new Date().toISOString(),
              userId: userId
          }));

          // Handle conversation
          const conversation = await handleConversation(dispatch, [studentPrompt], counselorPrompt, 5);
          conversation.forEach(log => {
              dispatch(addConversationLog(log));
          });

          // Handle post-survey
          const postAnswers = await handlePostSurvey(dispatch, [studentPrompt], postQuestions);
          dispatch(updatePostSurveyResults(postAnswers));

          // Set session end details
          dispatch(setSessionDetails({
              endTime: new Date().toISOString()
          }));
      } catch (error) {
          console.error('Failed to conduct therapy session:', error);
      }
  };
};