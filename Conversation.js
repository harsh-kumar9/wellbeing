...
const Conversation = ({ main, backchannelType, inputType }) => {
  ...
  const [inactive, setInactive] = useState(false); // Inactive state
  ...
  const backchannels = [
    ...
    const resetInactivityTimer = () => {
      ...
      setTimeout(() => {
        setInactive(true);
        setModalOpen(true); // Show modal after 1 min of inactivity
    ...
    useEffect(() => {
      ...
      // Set initial timeout for inactivity
  ...
  const processMessage = () => {
    if (inputValue.trim() !== "") {
      ...
      dispatch(convoActions.reset());
      dispatch(
        ask(langchain.prompt, langchain.bot_identifier, langchain.human_identifier, inputJSON)
      ).then((botResponse) => {
        setInputValue("");
        setLastBotMsg(botResponse["text"]);
      });
    }
  };
  ...
  const onCloseSurvey = () => {
    setShowSurvey(false);
    // Any additional cleanup actions can go here
  };

  return (
    <div className="conversation">
      <div className="convo-header-container">
        <h1>LLM for Wellbeing</h1>
        <button onClick={handleExitSession} className = "exit-session-button">
        <span className="visually-hidden">End Session</span>
        </button>
        {showSurvey && <PostSessionSurvey onClose={() => setShowSurvey(false)} />}
        <InactivityModal open={modalOpen} handleClose={() => setModalOpen(false)} />
      </div>
      ...
      <div className="convo-history-container" ref={convoHistoryRef}>
        {langchain.history.slice(0, -1).map((history, index) => (
          <div key={index}>
            ...
            <div className="bot-convo-container">
              <div className="bot-avatar-backchannel">
                <img src={bot_listen} id="bot_avatar_img" alt="Bot Status" />
              </div>
        ...
        {langchain.history.slice(-1).map((history, index) => (
          <div key={index}>
            ...
            <div className="bot-convo-container">
              <div className="bot-avatar-backchannel">
                <img src={isTyping ? bot_listen : bot_talk} id="bot_avatar_img" alt="Bot Status"/>
              </div>
              <div className="bot-convo">
                <p>{lastBotMsg}</p>
              </div>
...
export default ReactTimeout(Conversation);
